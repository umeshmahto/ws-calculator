...
class DJBMonthlyBillingCalculationSnapshotServiceTest {
	...
	@BeforeEach
	void setUp() {
		service = new DJBMonthlyBillingCalculationSnapshotService(calculationDao, billingCycleDao, new ObjectMapper());
	}
	...
	void shouldPersistConfiguredMinimumFromBillingMasterInsteadOfHardcoded25() throws Exception {
		...
		TariffCalculationResult water = TariffCalculationResult.builder().tariffId("DOMESTIC").category("DOMESTIC")
				.consumption(new BigDecimal("30")).waterVolumetricCharge(new BigDecimal("100"))
				.serviceCharge(new BigDecimal("200")).totalWaterCharge(new BigDecimal("300"))
				.slabCharges(Collections.emptyList()).build();

		SewerageCalculationResult sewerage = SewerageCalculationResult.builder()
				.regularSewerageCharge(new BigDecimal("60")).additionalSewerageCharge(BigDecimal.ZERO)
				.totalSewerageCharge(new BigDecimal("60")).regularRuleCode("SEWER_60_PERCENT").build();

		RebateCalculationResult rebate = RebateCalculationResult.builder().totalRebate(BigDecimal.ZERO)
				.rebateItems(Collections.emptyList()).explanation("No rebate").build();
		...
		String calculationId = service.persist(new RequestInfo(), cycle, new WaterConnection(), new Property(), water,
				sewerage, rebate, rule, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO,
				Collections.<String>emptyList(), new BigDecimal("360"));

		assertEquals(36, calculationId.length());

		ArgumentCaptor<DJBMonthlyBillingCalculation> captor = ArgumentCaptor
				.forClass(DJBMonthlyBillingCalculation.class);
		verify(calculationDao).save(captor.capture());

		DJBMonthlyBillingCalculation saved = captor.getValue();
		DJBMonthlyBillingStatement statement = new ObjectMapper().readValue(saved.getSnapshotjson(),
				DJBMonthlyBillingStatement.class);

		assertEquals(new BigDecimal("27"), statement.getBillingDecision().getMinimumBillingConsumption());
		assertEquals(Integer.valueOf(2), statement.getBillingDecision().getConfiguredAverageMaximumCycles());
		assertEquals(Integer.valueOf(2), statement.getBillingDecision().getConfiguredProvisionalMaximumCycles());
		assertEquals("DJB_MONTHLY_RULE", statement.getBillingDecision().getBillingRuleCode());
	}
	...
	void shouldPersistZroPendingSnapshotWithoutDemandOrBill() throws Exception {
		...
		cycle.setZrostatus(org.egov.wscalculation.djbmonthlybilling.model.enums.ZroStatus.PENDING);
		...
		String calculationId = service.persistPendingZro(new RequestInfo(), cycle, connection, property, rule);
		...
		assertEquals("ZRO_PENDING", saved.getStatus());
		assertEquals("PENDING", statement.getBillingCycle().getZroStatus());
		assertEquals("ONE_POINT_FIVE_X_ZRO", statement.getBillingDecision().getReasonCode());
		assertEquals(new BigDecimal("22.5"), statement.getOnePointFiveX().getThresholdConsumption());
		assertEquals(new BigDecimal("20"), statement.getOnePointFiveX().getMinimumZroConsumption());
		assertEquals("ZRO_PENDING", statement.getReconciliation().getStatus());
		org.junit.jupiter.api.Assertions.assertNull(statement.getDemand());
		org.junit.jupiter.api.Assertions.assertNull(statement.getBill());
	}
	...
	void shouldExposeCorrectionSupersededCyclesAndCalculatedAmounts() {
		...
		DJBMonthlyBillingCalculation previousCalculation = DJBMonthlyBillingCalculation.builder().id("CALC-AVG")
				.tenantid("dl.djb").billingcycleid("CYCLE-AVG").snapshotjson("{\"charges\":{\"netAmount\":100.00}}")
				.build();

		when(calculationDao.findById("dl.djb", "CALC-AVG")).thenReturn(previousCalculation);
		when(billingCycleDao.findCyclesForCorrection("dl.djb", current.getConnectionno(), 1000L,
				current.getBillingperiodto())).thenReturn(Collections.singletonList(previous));
		when(billingCycleDao.findPreviousOkByConnectionBefore("dl.djb", current.getConnectionno(),
				current.getBillingperiodto())).thenReturn(null);
		when(calculationDao.save(any(DJBMonthlyBillingCalculation.class))).thenReturn(1);

		TariffCalculationResult water = TariffCalculationResult.builder().tariffId("DOMESTIC").category("DOMESTIC")
				.consumption(new BigDecimal("38")).waterVolumetricCharge(new BigDecimal("200"))
				.serviceCharge(new BigDecimal("200")).totalWaterCharge(new BigDecimal("400"))
				.slabCharges(Collections.emptyList()).build();
		SewerageCalculationResult sewerage = SewerageCalculationResult.builder()
		...
		String id = service.persist(new RequestInfo(), current, new WaterConnection(), new Property(), water, sewerage,
				rebate, null, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, Collections.<String>emptyList(),
				new BigDecimal("520"));
