...
@Slf4j
@Component
public class GroupFDemandStrategy implements GroupDemandStrategy {

    private static final BigDecimal THRESHOLD_12500 = new BigDecimal("12500");
    private static final BigDecimal OCCUPANCY_DENOMINATOR_4_5 = new BigDecimal("4.5");

    @Override
    public boolean supports(String subCategoryCode, String parentUsageCode) {
        if (subCategoryCode == null) return false;
        return subCategoryCode.startsWith("F-") || "MERCANTILE".equalsIgnoreCase(parentUsageCode) || "RETAIL".equalsIgnoreCase(parentUsageCode);
    }
    ...
    public void processGroupDemand(WaterDemandResult result, Map<String, Object> matchedNorm, Map<String, BigDecimal> contextVariables) {
        log.info("Executing Group F (Mercantile / Retail Shops F-1 & F-2) Demand Calculation Logic");

        String code = result.getMatchedNormCode();
        BigDecimal rawOccupancy = result.getCalculatedOccupancy();

        BigDecimal resolvedOccupancy = resolveMercantileOccupancy(rawOccupancy, contextVariables);
        ...
        if ("POTABLE_ONLY_IF_OVER_12500".equalsIgnoreCase(ifcBasis) && initialTotalDemand.compareTo(THRESHOLD_12500) > 0) {
            ...
            log.info("Group F ({}) 12,500 LPD Threshold Triggered: Total LPD {} > 12500. Applied Potable LPCD Rate {}", code, initialTotalDemand, chosenLpcd);
        }
    ...
    /**
     * Resolves Occupancy for Group F (F-1 & F-2):
     */
    private BigDecimal resolveMercantileOccupancy(BigDecimal rawOccupancy, Map<String, BigDecimal> contextVars) {
        BigDecimal farArea = contextVars.getOrDefault("far_area", BigDecimal.ZERO);

        if (farArea.compareTo(BigDecimal.ZERO) > 0) {
            BigDecimal resolved = farArea.divide(OCCUPANCY_DENOMINATOR_4_5, 0, RoundingMode.CEILING);
            log.info("Group F Occupancy resolved from FAR Area ({}/4.5): {}", farArea, resolved);
            return resolved;
        }
