...
public class WSCalculatorQueryBuilder {
	...
	private final static String Query = "SELECT mr.id, mr.connectionNo as connectionId, mr.billingPeriod, mr.meterStatus, mr.lastReading, mr.lastReadingDate, mr.currentReading,"
			...
			+ " mr.createdTime as mr_createdTime, mr.lastModifiedTime as mr_lastModifiedTime,"
			+ " bc.readingqualitycode as mr_readingQualityCode, COALESCE(bc.billingconsumption, bc.actualconsumption) as mr_consumption,"
			+ " bc.id as mr_billingCycleId, bc.status as mr_billingCycleStatus"
			+ " FROM eg_ws_meterreading mr"
			+ " LEFT JOIN eg_ws_billingcycle bc ON bc.tenantid = mr.tenantid AND bc.meterreadingid = mr.id";
	...
	private final static String Locality_Query="select mr.connectionNo as connectionId,mr.id,ms.property_id,pa.locality, mr.billingPeriod, mr.meterStatus, mr.lastReading, mr.lastReadingDate, mr.currentReading,mr.currentReadingDate,mr.createdBy as mr_createdBy, mr.tenantid, mr.lastModifiedBy as mr_lastModifiedBy,"
			...
			+ " bc.readingqualitycode as mr_readingQualityCode, COALESCE(bc.billingconsumption, bc.actualconsumption) as mr_consumption,"
			+ " bc.id as mr_billingCycleId, bc.status as mr_billingCycleStatus"
			+ " from (select * from eg_ws_meterreading mr where (mr.connectionNo,mr.createdTime) in (select connectionNo, max(createdTime) from eg_ws_meterreading group by connectionNo)) mr"
			+ " LEFT JOIN eg_ws_billingcycle bc ON bc.tenantid = mr.tenantid AND bc.meterreadingid = mr.id"
	...
	public String getConnectionNumber(String tenantId, String consumerCode,String connectionType, List<Object> preparedStatement,Long fromDate, Long toDate) {
		...
		addClauseIfRequired(preparedStatement, query);
		query.append(" conn.connectionno NOT IN (select distinct(consumercode) from egbs_demand_v1 dmd where (dmd.taxperiodfrom >= ? and dmd.taxperiodto <= ?) and businessservice = 'WS' and tenantid=? and consumercode = ?)");
		preparedStatement.add(fromDate);
		preparedStatement.add(toDate);
		preparedStatement.add(tenantId);
