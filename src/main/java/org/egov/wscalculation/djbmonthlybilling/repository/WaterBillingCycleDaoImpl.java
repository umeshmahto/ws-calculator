...
public class WaterBillingCycleDaoImpl implements WaterBillingCycleDao {
	...
	public int save(WaterBillingCycle c) {
		return jdbcTemplate.update(queryBuilder.insertBillingCycle(), c.getId(), c.getTenantid(), c.getConnectionno(),
				c.getBillingperiodfrom(), c.getBillingperiodto(), c.getMeterreadingid(), c.getReadingqualitycode(),
				value(c.getBillingbasis()), c.getPreviousokreading(), c.getPreviousokreadingdate(),
				...
				value(c.getCorrectionstatus()), value(c.getStatus()), c.getCreatedby(), c.getCreatedtime(),
				c.getLastmodifiedby(), c.getLastmodifiedtime());
	}

	@Override
	public int update(WaterBillingCycle c) {
		return jdbcTemplate.update(queryBuilder.updateBillingCycle(), c.getConnectionno(), c.getBillingperiodfrom(),
				c.getBillingperiodto(), c.getMeterreadingid(), c.getReadingqualitycode(), value(c.getBillingbasis()),
				c.getPreviousokreading(), c.getPreviousokreadingdate(), c.getCurrentreading(),
				c.getCurrentreadingdate(), c.getActualconsumption(), c.getAverageconsumption(),
				c.getBillingconsumption(), c.getPreviousconsumption(), c.getDeviationfactor(), c.getOnepointfivexflag(),
				c.getAveragecyclecount(), c.getProvisionalcyclecount(), value(c.getZrostatus()), c.getZroremarks(),
				c.getZroactionby(), c.getZroactiondate(), c.getCalculationid(), c.getDemandid(), c.getBillid(),
	...
	@Override
	public WaterBillingCycle findById(String t, String id) {
		List<WaterBillingCycle> r = jdbcTemplate.query(queryBuilder.findById(), rowMapper, t, id);
		return r.isEmpty() ? null : r.get(0);
	}

	@Override
	public WaterBillingCycle findByConnectionAndPeriod(String t, String c, Long f, Long to) {
		List<WaterBillingCycle> r = jdbcTemplate.query(queryBuilder.findByConnectionAndPeriod(), rowMapper, t, c, f,
				to);
		return r.isEmpty() ? null : r.get(0);
	}

	@Override
	public List<WaterBillingCycle> findOverlappingCycles(String t, String c, Long from, Long to) {
	...
	@Override
	public WaterBillingCycle findLatestByConnection(String t, String c) {
		List<WaterBillingCycle> r = jdbcTemplate.query(queryBuilder.findLatestByConnection(), rowMapper, t, c);
		return r.isEmpty() ? null : r.get(0);
	}

	@Override
	public WaterBillingCycle findLatestOkByConnection(String t, String c) {
		List<WaterBillingCycle> r = jdbcTemplate.query(queryBuilder.findLatestOkByConnection(), rowMapper, t, c);
		return r.isEmpty() ? null : r.get(0);
	}

	@Override
	public List<WaterBillingCycle> findPreviousActualCycles(String t, String c, Long p, int l) {
	...
	@Override
	public List<WaterBillingCycle> findCyclesForCorrection(String t, String c, Long f, Long to) {
	...
	@Override
	public WaterBillingCycle findPreviousOkByConnectionBefore(String tenantId, String connectionNo,
			Long beforeBillingPeriodTo) {

		List<WaterBillingCycle> result = jdbcTemplate.query(queryBuilder.findPreviousOkByConnectionBefore(), rowMapper,
				tenantId, connectionNo, beforeBillingPeriodTo);
