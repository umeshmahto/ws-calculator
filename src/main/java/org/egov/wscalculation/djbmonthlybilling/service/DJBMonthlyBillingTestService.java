...
public class DJBMonthlyBillingTestService {
    ...
    public DJBMonthlyBillingTestService(
    ...
    public DJBMonthlyBillingTestResponse calculate(
            DJBMonthlyBillingTestRequest request) {
        ...
        TariffCalculationResult water =
                tariffCalculationService.calculate(
                        ...
                        request.getTariffCategory(),
                        tariffs);

        List<DJBMonthlySewerageRule> sewerRules =
                masterProvider.getSewerageRules(requestInfo, tenantId);

        List<DJBAdditionalSewerageCharge> additionalSewerRules =
                masterProvider.getAdditionalSewerageCharges(
                        requestInfo, tenantId);

        SewerageCalculationContext sewerContext =
                SewerageCalculationContext.builder()
                        .waterVolumetricCharge(
                                water.getWaterVolumetricCharge())
                        .waterConnectionAvailable(
                                request.isWaterConnectionAvailable())
                        .sewerConnectionAvailable(
                                request.isSewerConnectionAvailable())
                        .additionalWaterSource(
                                request.isAdditionalWaterSource())
                        .consumerCategory(
                                request.getConsumerCategory())
                        .propertyUsage(
                                request.getPropertyUsage())
                        .builtUpAreaSqm(
                                request.getBuiltUpAreaSqm())
                        .numberOfRooms(
        ...
        RebateCalculationContext rebateContext =
                RebateCalculationContext.builder()
                        ...
                        .consumerType(request.getConsumerCategory())
                        .propertyCategory(request.getConsumerCategory())
                        .connectionType(request.getTariffCategory())
                        .bulkConnection(false)
                        .propertyAreaSqm(request.getBuiltUpAreaSqm())
                        .functionalRwh(false)
                        .functionalWastewaterRecycling(false)
                        .totalBillBeforeRebate(totalBeforeRebate)
                        .freeWaterEligibleAmount(
                                request.getFreeWaterEligibleAmount())
                        .build();
        ...
        BigDecimal netAmount = totalBeforeRebate.subtract(rebate.getTotalRebate()).setScale(MONEY_SCALE, RoundingMode.HALF_UP);

        return DJBMonthlyBillingTestResponse.builder()
                .tenantId(tenantId)
                .connectionNo(request.getConnectionNo())
                .readingQualityCode(request.getReadingQualityCode())
                .billingBasis(request.getBillingBasis() == null
                        ? null
                        : request.getBillingBasis().toString())
                .consumption(request.getConsumption())
                .previousConsumption(request.getPreviousConsumption())
                .water(water)
                .sewerage(sewerage)
                .rebate(rebate)
                .totalBeforeRebate(totalBeforeRebate)
                .netAmountAfterRebate(netAmount)
                .build();
    }
