...
public class DJBMonthlyBillingFetchService {
	...
	public DJBMonthlyBillingFetchService(WaterBillingCycleDao billingCycleDao,
	...
	public DJBMonthlyBillingStatementResponse fetch(DJBMonthlyBillingFetchRequest request) {
		...
		return DJBMonthlyBillingStatementResponse.builder().billingStatement(statement).build();
	}
	...
	private void refreshCurrentCycleState(WaterBillingCycle cycle, DJBMonthlyBillingStatement statement) {
		...
		if (statement.getDemand() == null) {
			statement.setDemand(DJBMonthlyBillingStatement.DemandSummary.builder()
					.created(StringUtils.hasText(cycle.getDemandid())).id(cycle.getDemandid())
					.amount(statement.getCharges() == null ? null : statement.getCharges().getNetAmount()).build());
		} else {
			statement.getDemand()
					.setId(cycle.getDemandid() == null ? statement.getDemand().getId() : cycle.getDemandid());
			statement.getDemand().setCreated(StringUtils.hasText(cycle.getDemandid()));
			statement.getDemand()
					.setAmount(statement.getCharges() == null ? null : statement.getCharges().getNetAmount());
		}
		if (statement.getBill() == null) {
			statement.setBill(DJBMonthlyBillingStatement.BillSummary.builder()
					.generated(StringUtils.hasText(cycle.getBillid())).id(cycle.getBillid())
	...
	private void mergeBillDetails(RequestInfo requestInfo, String tenantId, WaterBillingCycle cycle,
		...
		if (statement.getBill() == null) {
			statement.setBill(
					DJBMonthlyBillingStatement.BillSummary.builder().generated(true).id(cycle.getBillid()).build());
		}
		...
		statement.getBill()
				.setCalculatedAmount(statement.getCharges() == null ? null : statement.getCharges().getNetAmount());
		...
		if (actualAmount == null || calculated == null) {
			...
			statement.setReconciliation(DJBMonthlyBillingStatement.Reconciliation.builder().calculatedAmount(calculated)
					.billedAmount(actualAmount).status("NOT_RECONCILED").build());
			return;
		}
		...
		statement.setReconciliation(DJBMonthlyBillingStatement.Reconciliation.builder().calculatedAmount(calculated)
				.billedAmount(actualAmount).difference(difference).status(reconciliationStatus).build());
	}
	...
	private Map<String, Object> fetchBillSearch(RequestInfo requestInfo, String tenantId, String billId) {
		StringBuilder url = new StringBuilder(billingServiceHost).append(billSearchEndpoint)
				...
				.append(WSCalculationConstant.SEPARATER).append("billId=").append(billId);
	...
	private BigDecimal extractCycleBillAmount(Map<String, Object> bill, String billingCycleId,
			Map<String, Object> cycleDetail) {
		if (cycleDetail != null) {
			Map<String, Object> additionalDetails = asMap(getIgnoreCase(cycleDetail, "additionalDetails"));
			BigDecimal netAmount = toBigDecimal(
					additionalDetails == null ? null : getIgnoreCase(additionalDetails, "netAmount"));
			if (netAmount != null) {
