...
public class BillingBasisService {
    ...
    public BillingBasisDecision determineBillingBasis(String tenantId, String connectionNo,
        ...
        BigDecimal previousConsumption = findPreviousConsumption(
                tenantId, connectionNo, currentCycle.getBillingperiodto());

        if ("OK".equalsIgnoreCase(rqc.getCode())
                || "ACTUAL".equalsIgnoreCase(rqc.getBillingTreatment())) {
            return BillingBasisDecision.builder()
                    .billingBasis(BillingBasis.ACTUAL)
                    .readingQualityCode(code)
                    .averageCycleCount(0)
        ...
        if (!"AVERAGE".equalsIgnoreCase(rqc.getBillingTreatment())) {
            throw new IllegalArgumentException(
                    "Unsupported DJB RQC billing treatment: " + rqc.getBillingTreatment());
        }

        // Query recent cycles, not only AVERAGE rows. A non-average cycle must terminate
        // the consecutive streak; otherwise an isolated old AVERAGE would be counted.
        List<WaterBillingCycle> recent = billingCycleDao.findCyclesForConnection(
                tenantId, connectionNo, currentCycle.getBillingperiodto(), 24);

        int averageCount = 0;
        int provisionalCount = 0;
        for (WaterBillingCycle cycle : recent) {
            if (BillingBasis.AVERAGE.equals(cycle.getBillingbasis())) {
                if (provisionalCount > 0) break;
                averageCount++;
            } else if (BillingBasis.PROVISIONAL.equals(cycle.getBillingbasis())) {
                if (averageCount > 0) break;
                provisionalCount++;
            } else {
                break;
            }
        ...
        return BillingBasisDecision.builder()
                ...
                .readingQualityCode(code)
                .averageCycleCount(averageCount + 1)
                .provisionalCycleCount(provisionalCount)
                .requiresZro(false)
                .actualReadingAvailable(false)
                .previousConsumption(previousConsumption)
                .build();
    }

    private BigDecimal findPreviousConsumption(String tenantId, String connectionNo, Long periodTo) {
        List<WaterBillingCycle> cycles = billingCycleDao.findPreviousActualCycles(
                tenantId, connectionNo, periodTo, 1);
