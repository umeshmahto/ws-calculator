...
public class DJBMonthlyBillingCalculationSnapshotService {
	...
	public DJBMonthlyBillingCalculationSnapshotService(DJBMonthlyBillingCalculationDao calculationDao,
			WaterBillingCycleDao billingCycleDao, ObjectMapper objectMapper) {
		this.calculationDao = calculationDao;
		this.billingCycleDao = billingCycleDao;
		this.objectMapper = objectMapper;
	}

	public String persist(RequestInfo requestInfo, WaterBillingCycle cycle, WaterConnection connection,
			Property property, TariffCalculationResult water, SewerageCalculationResult sewerage,
			RebateCalculationResult rebate, DJBMonthlyBillingRule billingRule, BigDecimal appliedPaidAdjustment,
		...
		DJBMonthlyBillingStatement statement = buildStatement(cycle, connection, property, water, sewerage, rebate,
				billingRule, appliedPaidAdjustment, residualPaidCredit, carriedForwardCreditApplied,
		...
		try {
			...
			throw new IllegalStateException("Unable to serialize DJB billing calculation snapshot for " + cycle.getId(),
					ex);
		}
	...
	/**
	 * Persists an auditable statement for a domestic 1.5x cycle that is waiting for
	 * ZRO verification. No demand or bill is created at this stage; the snapshot
	 * records the decision context so the fetch API remains useful while billing is
	 * blocked.
	 */
	public String persistPendingZro(RequestInfo requestInfo, WaterBillingCycle cycle, WaterConnection connection,
			Property property, DJBMonthlyBillingRule billingRule) {

		if (StringUtils.hasText(cycle.getCalculationid())) {
			updateStatus(cycle.getTenantid(), cycle.getCalculationid(), "ZRO_PENDING");
			return cycle.getCalculationid();
		}
		...
		DJBMonthlyBillingStatement statement = DJBMonthlyBillingStatement.builder()
				.consumer(DJBMonthlyBillingStatement.Consumer.builder().tenantId(cycle.getTenantid())
						.connectionNo(cycle.getConnectionno())
						.tariffCategory(connection == null ? null : connection.getConnectionCategory())
				...
				.billingCycle(DJBMonthlyBillingStatement.BillingCycle.builder().id(cycle.getId())
						.periodFrom(cycle.getBillingperiodfrom()).periodTo(cycle.getBillingperiodto())
						...
						.correctionStatus(
								cycle.getCorrectionstatus() == null ? null : cycle.getCorrectionstatus().toString())
						.status("CALCULATED").averageCycleCount(cycle.getAveragecyclecount())
						.provisionalCycleCount(cycle.getProvisionalcyclecount())
						.zroStatus(cycle.getZrostatus() == null ? "PENDING" : cycle.getZrostatus().toString())
						.zroRemarks(cycle.getZroremarks()).build())
				.reading(DJBMonthlyBillingStatement.Reading.builder().previousReading(cycle.getPreviousokreading())
						.previousReadingDate(cycle.getPreviousokreadingdate()).currentReading(cycle.getCurrentreading())
				...
				.billingDecision(DJBMonthlyBillingStatement.BillingDecision.builder()
						.basis(cycle.getBillingbasis() == null ? null : cycle.getBillingbasis().toString())
						.reasonCode("ONE_POINT_FIVE_X_ZRO")
						.reason("DJB 1.5x high-consumption verification is pending; demand and bill generation are withheld until ZRO action.")
						.actualReadingAvailable(cycle.getActualconsumption() != null)
						.historicalAverage(cycle.getAverageconsumption())
						.minimumBillingConsumption(
								billingRule == null || billingRule.getMinimumPostAverageConsumptionKl() == null ? null
										: BigDecimal
												.valueOf(billingRule.getMinimumPostAverageConsumptionKl().longValue()))
						.averageCycleCount(cycle.getAveragecyclecount())
						.provisionalCycleCount(cycle.getProvisionalcyclecount())
						.billingRuleCode(billingRule == null ? null : billingRule.getCode())
						.configuredAverageMaximumCycles(
								billingRule == null ? null : billingRule.getAverageMaximumCycles())
						.configuredProvisionalMaximumCycles(
				...
				.adjustments(DJBMonthlyBillingStatement.Adjustments.builder()
						.paidCorrectionApplied(BigDecimal.ZERO.setScale(MONEY_SCALE))
						.residualPaidCreditCreated(BigDecimal.ZERO.setScale(MONEY_SCALE))
						.carriedForwardCreditApplied(BigDecimal.ZERO.setScale(MONEY_SCALE))
						.carriedForwardCreditAllocationIds(new ArrayList<String>()).build())
				.correction(DJBMonthlyBillingStatement.CorrectionSummary.builder().required(false)
						.status(cycle.getCorrectionstatus() == null ? null : cycle.getCorrectionstatus().toString())
						.build())
				.reconciliation(DJBMonthlyBillingStatement.Reconciliation.builder().calculatedAmount(null)
						.billedAmount(null).difference(null).status("ZRO_PENDING").build())
				.demand(null).bill(null)
				.audit(DJBMonthlyBillingStatement.Audit.builder().calculationId(calculationId)
						.engineVersion(ENGINE_VERSION).calculatedAt(now).calculatedBy(actor(requestInfo))
						.calculationStatus("ZRO_PENDING")
						.billingRuleCode(billingRule == null ? null : billingRule.getCode())
						.tariffId(connection == null ? null : connection.getConnectionCategory())
						.sewerageRuleCodes(new ArrayList<String>()).rebateCodes(new ArrayList<String>()).build())
				.build();
		...
		try {
			...
			throw new IllegalStateException(
					"Unable to serialize DJB ZRO pending calculation snapshot for " + cycle.getId(), ex);
		}

		DJBMonthlyBillingCalculation calculation = DJBMonthlyBillingCalculation.builder().id(calculationId)
				.tenantid(cycle.getTenantid()).billingcycleid(cycle.getId()).connectionno(cycle.getConnectionno())
				.engineversion(ENGINE_VERSION).status("ZRO_PENDING").calculatedtime(now)
				.calculatedby(actor(requestInfo)).snapshotjson(snapshotJson).build();

		if (calculationDao.save(calculation) != 1) {
			throw new IllegalStateException(
					"Failed to persist DJB ZRO pending calculation snapshot for " + cycle.getId());
		}
		return calculationId;
	}
	...
	public void updateStatus(String tenantId, String calculationId, String status) {
		...
		if (calculationDao.updateStatus(tenantId, calculationId, status) != 1) {
			throw new IllegalStateException(
					"Failed to update DJB billing calculation snapshot status for " + calculationId);
		}
	}
	...
	public DJBMonthlyBillingStatement read(String tenantId, String calculationId) {
		...
		try {
			DJBMonthlyBillingStatement statement = objectMapper.readValue(calculation.getSnapshotjson(),
					DJBMonthlyBillingStatement.class);
	...
	private DJBMonthlyBillingStatement buildStatement(WaterBillingCycle cycle, WaterConnection connection,
			Property property, TariffCalculationResult water, SewerageCalculationResult sewerage,
			RebateCalculationResult rebate, DJBMonthlyBillingRule billingRule, BigDecimal appliedPaidAdjustment,
			BigDecimal residualPaidCredit, BigDecimal carriedForwardCreditApplied,
			List<String> carriedForwardCreditAllocationIds, BigDecimal netAmount, String calculationId,
			long calculatedAt, String calculatedBy) {
		...
		if (BillingBasis.ACTUAL.equals(cycle.getBillingbasis())
			...
			reasonCode = BillingBasis.CORRECTED_ACTUAL.equals(cycle.getBillingbasis()) ? "CORRECTION_COMPLETED"
					: "RQC_OK";
		} else if (BillingBasis.AVERAGE.equals(cycle.getBillingbasis())) {
			decisionReason = "Meter reading quality code " + cycle.getReadingqualitycode()
					+ " is billed on the DJB average basis.";
		...
		if (rebate != null && rebate.getRebateItems() != null) {
			for (RebateItem item : rebate.getRebateItems()) {
				rebateLines.add(DJBMonthlyBillingStatement.RebateLine.builder().code(item.getCode())
						.name(item.getName()).rate(item.getRate()).baseAmount(item.getBaseAmount())
						.rebateAmount(item.getRebateAmount()).explanation(item.getExplanation()).build());
			}
		}
		...
		if (billingRule != null && (cycle.getBillingbasis() == BillingBasis.PROVISIONAL
				|| cycle.getBillingbasis() == BillingBasis.AVERAGE)) {
			configuredMinimum = billingRule.getMinimumPostAverageConsumptionKl() == null ? null
					: BigDecimal.valueOf(billingRule.getMinimumPostAverageConsumptionKl().longValue());
		}
		...
		if (sewerage.getRegularRuleCode() != null)
			sewerageRuleCodes.add(sewerage.getRegularRuleCode());
		if (sewerage.getAdditionalRuleCode() != null)
			sewerageRuleCodes.add(sewerage.getAdditionalRuleCode());
		...
		for (DJBMonthlyBillingStatement.RebateLine line : rebateLines) {
			if (line.getCode() != null)
				rebateCodes.add(line.getCode());
		}

		return DJBMonthlyBillingStatement.builder()
				.consumer(DJBMonthlyBillingStatement.Consumer.builder().tenantId(cycle.getTenantid())
						.connectionNo(cycle.getConnectionno()).tariffCategory(water.getCategory())
						.propertyUsage(property == null ? null : property.getUsageCategory())
						.propertyAreaSqm(property == null ? null : property.getSuperBuiltUpArea()).build())
				.billingCycle(DJBMonthlyBillingStatement.BillingCycle.builder().id(cycle.getId())
						.periodFrom(cycle.getBillingperiodfrom()).periodTo(cycle.getBillingperiodto())
				...
				.billingDecision(DJBMonthlyBillingStatement.BillingDecision.builder().basis(billingBasis)
						.reasonCode(reasonCode).reason(decisionReason)
						.actualReadingAvailable(cycle.getActualconsumption() != null)
						.historicalAverage(cycle.getAverageconsumption()).minimumBillingConsumption(configuredMinimum)
						.averageCycleCount(cycle.getAveragecyclecount())
						.provisionalCycleCount(cycle.getProvisionalcyclecount())
						.billingRuleCode(billingRule == null ? null : billingRule.getCode())
						.configuredAverageMaximumCycles(
				...
				.charges(DJBMonthlyBillingStatement.Charges.builder()
						...
						.sewerage(DJBMonthlyBillingStatement.SewerageCharges.builder()
								...
								.explanation(sewerage.getExplanation()).build())
						.rebates(DJBMonthlyBillingStatement.RebateCharges.builder()
								.totalRebate(rebate == null ? BigDecimal.ZERO.setScale(MONEY_SCALE)
										: rebate.getTotalRebate())
								.lines(rebateLines)
								.explanation(
										rebate == null ? "No DJB monthly rebate applicable" : rebate.getExplanation())
								.build())
				...
				.adjustments(DJBMonthlyBillingStatement.Adjustments.builder()
						...
						.carriedForwardCreditAllocationIds(
								carriedForwardCreditAllocationIds == null ? new ArrayList<String>()
										: carriedForwardCreditAllocationIds)
						.build())
				.correction(correction)
				.reconciliation(DJBMonthlyBillingStatement.Reconciliation.builder().calculatedAmount(netAmount)
						.status("PENDING_BILL").build())
				.demand(null).bill(null)
				.audit(DJBMonthlyBillingStatement.Audit.builder().calculationId(calculationId)
						.engineVersion(ENGINE_VERSION).calculatedAt(calculatedAt).calculatedBy(calculatedBy)
						.calculationStatus("CALCULATED")
						.billingRuleCode(billingRule == null ? null : billingRule.getCode())
						.tariffId(water.getTariffId()).sewerageRuleCodes(sewerageRuleCodes).rebateCodes(rebateCodes)
						.build())
				.build();
	}

	private String buildDecisionReason(WaterBillingCycle cycle, DJBMonthlyBillingRule billingRule, String defaultReason,
	...
	private DJBMonthlyBillingStatement.CorrectionSummary buildCorrectionSummary(WaterBillingCycle cycle) {
		boolean required = cycle.getCorrectionstatus() != null && cycle.getPreviousokreadingdate() != null
				&& (BillingBasis.CORRECTED_ACTUAL.equals(cycle.getBillingbasis())
						|| cycle.getCorrectionstatus().toString().contains("COMPLETED"));
		...
		List<WaterBillingCycle> cycles = billingCycleDao.findCyclesForCorrection(cycle.getTenantid(),
				cycle.getConnectionno(), cycle.getPreviousokreadingdate(), cycle.getBillingperiodto());
		if (cycles != null) {
			for (WaterBillingCycle previous : cycles) {
				BigDecimal amount = readCalculatedAmount(previous);
				DJBMonthlyBillingStatement.CorrectionCycle correctionCycle = DJBMonthlyBillingStatement.CorrectionCycle
						.builder().billingCycleId(previous.getId())
						.billingBasis(previous.getBillingbasis() == null ? null : previous.getBillingbasis().toString())
						.billingConsumption(previous.getBillingconsumption()).demandId(previous.getDemandid())
						.billId(previous.getBillid()).calculatedNetAmount(amount).build();
				if (amount != null)
					previouslyCalculatedAmount = previouslyCalculatedAmount.add(amount);
		...
		BigDecimal correctedConsumption = cycle.getCurrentreading() != null && cycle.getPreviousokreading() != null
				? cycle.getCurrentreading().subtract(cycle.getPreviousokreading())
				: cycle.getActualconsumption();

		return DJBMonthlyBillingStatement.CorrectionSummary.builder().required(true)
				.status(cycle.getCorrectionstatus().toString()).reasonCode("OK_AFTER_ESTIMATED_CYCLES")
				.reason("Current valid OK reading corrected prior average/provisional billing cycles.")
				.previousOkBillingCycleId(findPreviousOkCycleId(cycle)).currentBillingCycleId(cycle.getId())
				.previousOkReading(cycle.getPreviousokreading()).currentReading(cycle.getCurrentreading())
				.correctedConsumption(correctedConsumption).supersededCycles(superseded)
				.previouslyCalculatedAmount(previouslyCalculatedAmount).build();
	}

	private String findPreviousOkCycleId(WaterBillingCycle cycle) {
		WaterBillingCycle previous = billingCycleDao.findPreviousOkByConnectionBefore(cycle.getTenantid(),
				cycle.getConnectionno(), cycle.getBillingperiodto());
	...
	private BigDecimal readCalculatedAmount(WaterBillingCycle cycle) {
		...
		if (previous == null || !StringUtils.hasText(previous.getSnapshotjson()))
			return null;
		try {
			DJBMonthlyBillingStatement statement = objectMapper.readValue(previous.getSnapshotjson(),
					DJBMonthlyBillingStatement.class);
