...
public class SewerageCalculationService {
    ...
    public SewerageCalculationResult calculate(
            SewerageCalculationContext context,
            List<DJBMonthlySewerageRule> regularRules,
            List<DJBAdditionalSewerageCharge> additionalRules) {
        ...
        AdditionalChargeSelection additional =
                calculateAdditional(context, additionalRules);

        BigDecimal total = regular.add(additional.amount)
        ...
        if (total.signum() == 0) {
            ...
            explanation = "Regular sewerage=" + regular
                    + ", additional sewerage=" + additional.amount;
        }

        return SewerageCalculationResult.builder()
                .regularSewerageCharge(regular)
                .additionalSewerageCharge(additional.amount)
                .totalSewerageCharge(total)
                .regularRuleCode(findRegularRuleCode(context, regularRules))
                .additionalRuleCode(additional.ruleCode)
                .explanation(explanation)
                .build();
    }

    private BigDecimal calculateRegular(
            SewerageCalculationContext context,
            List<DJBMonthlySewerageRule> rules) {

        if (!context.isSewerConnectionAvailable()
                || rules == null || rules.isEmpty()) {
            return moneyZero();
        }

        if (context.isWaterConnectionAvailable()) {
            DJBMonthlySewerageRule rule = rules.stream()
            ...
            if (rule == null
                    ...
                    || context.getWaterVolumetricCharge() == null) {
                return moneyZero();
            }

            return context.getWaterVolumetricCharge()
        ...
        return rules.stream()
                ...
                .filter(r -> withinArea(r, context.getBuiltUpAreaSqm()))
                .map(DJBMonthlySewerageRule::getAmount)
                .filter(Objects::nonNull)
                .findFirst()
    ...
    private AdditionalChargeSelection calculateAdditional(
            SewerageCalculationContext context,
            List<DJBAdditionalSewerageCharge> rules) {

        if (!context.isAdditionalWaterSource()
                || rules == null || rules.isEmpty()) {
        ...
        List<DJBAdditionalSewerageCharge> matches = rules.stream()
                .filter(Objects::nonNull)
                .filter(r -> !Boolean.FALSE.equals(r.getActive()))
        ...
        for (DJBAdditionalSewerageCharge rule : matches) {
            ...
            return new AdditionalChargeSelection(
                    calculateAdditionalRuleAmount(rule, context),
                    rule.getCode());
        }
    ...
    private BigDecimal calculateAdditionalRuleAmount(
        ...
        if ("BASE_PLUS_PER_BLOCK".equals(type)) {
            Integer count = context.getNumberOfRooms() != null
                    ? context.getNumberOfRooms()
                    : context.getNumberOfBeds();

            if (count == null) {
                throw new IllegalArgumentException(
                        "Rooms/beds are required for block-based additional sewerage");
            }

            Integer start = rule.getFromRooms() != null
                    ? rule.getFromRooms()
                    : rule.getFromBeds();

            int blockSize = rule.getBlockSize() == null || rule.getBlockSize() <= 0
                    ? 50 : rule.getBlockSize();
        ...
        throw new IllegalArgumentException(
                "Unsupported additional sewerage charge type: "
                        + rule.getChargeType());
    }
    ...
    private boolean rangeMatches(
        ...
        Integer from = rule.getFromRooms() != null
                ? rule.getFromRooms() : rule.getFromBeds();

        Integer to = rule.getToRooms() != null
                ? rule.getToRooms() : rule.getToBeds();
    ...
    private boolean withinArea(
        ...
        if (rule.getMinBuiltUpAreaSqm() != null
                && area.compareTo(rule.getMinBuiltUpAreaSqm()) < 0) {
            return false;
        }
        return rule.getMaxBuiltUpAreaSqm() == null
                || area.compareTo(rule.getMaxBuiltUpAreaSqm()) <= 0;
    }

    private String findRegularRuleCode(
            SewerageCalculationContext context,
            List<DJBMonthlySewerageRule> rules) {
        ...
        return rules.stream()
                ...
                .filter(r -> "ALL".equalsIgnoreCase(normalize(r.getCategory()))
                        || category.equals(normalize(r.getCategory())))
                .filter(r -> withinArea(r, context.getBuiltUpAreaSqm()))
                .map(DJBMonthlySewerageRule::getCode)
                .findFirst()
                .orElse(null);
    }

    private BigDecimal rangeStart(DJBAdditionalSewerageCharge rule) {
        Integer value = rule.getFromRooms() != null
                ? rule.getFromRooms() : rule.getFromBeds();
    ...
    private static class AdditionalChargeSelection {
        ...
        private AdditionalChargeSelection(
                BigDecimal amount,
                String ruleCode) {
            this.amount = amount;
