...
public class ConsumptionService {
    ...
    public ConsumptionResult calculate(String tenantId, String connectionNo,
            WaterBillingCycle currentCycle, BillingBasisDecision decision, DJBMonthlyBillingRule rule) {
        ...
        if (BillingBasis.ACTUAL.equals(decision.getBillingBasis())) {
            ...
            return ConsumptionResult.builder()
                    ...
                    .onePointFiveX(isOnePointFiveX(actual, decision.getPreviousConsumption(), rule))
                    .build();
        }

        BigDecimal average = calculateHistoricalAverage(
                tenantId, connectionNo, currentCycle.getBillingperiodto(), rule.getAverageLookbackMonths());
        if (average == null) {
    ...
    public BigDecimal calculateHistoricalAverage(String tenantId, String connectionNo,
            Long periodTo, Integer lookbackMonths) {
        List<WaterBillingCycle> cycles = billingCycleDao.findPreviousActualCycles(
                tenantId, connectionNo, periodTo, lookbackMonths);
    ...
    private boolean isOnePointFiveX(BigDecimal current, BigDecimal previous, DJBMonthlyBillingRule rule) {
        ...
        return current.compareTo(previous.multiply(factor, MC)) > 0
