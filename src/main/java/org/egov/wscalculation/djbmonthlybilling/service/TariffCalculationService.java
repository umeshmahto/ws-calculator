 ...
 *
 * Responsibilities:
 * - Resolve the DJB tariff by consumer category.
 * - Apply progressive slab rates.
 * - Select the applicable monthly service charge.
 *
 * This class does not create tax-head estimates, demands or bills.
 * Existing EstimationService is intentionally untouched.
 */
@Service
public class TariffCalculationService {
    ...
    public TariffCalculationResult calculate(
        ...
        DJBMonthlyWaterTariff tariff =
                resolveTariff(requestedCategory, tariffs);

        if (tariff == null) {
            throw new IllegalArgumentException(
                    "DJB monthly tariff not configured for category: "
                            + requestedCategory);
        }

        List<DJBMonthlyWaterTariffSlab> slabs =
                sortedActiveSlabs(tariff);

        if (slabs.isEmpty()) {
            throw new IllegalArgumentException(
                    "No active slabs configured for DJB tariff: "
                            + tariff.getId());
        }
        ...
        for (DJBMonthlyWaterTariffSlab slab : slabs) {
            ...
            if (to != null && to.compareTo(from) <= 0) {
                throw new IllegalArgumentException(
                        "Invalid DJB tariff slab: from=" + from
                                + ", to=" + to);
            }

            BigDecimal units = calculateUnitsInSlab(
                    consumption, from, to);
            ...
            if (slab.getRatePerKl() == null
                    || slab.getRatePerKl().signum() < 0) {
                throw new IllegalArgumentException(
                        "Invalid rate in DJB tariff slab");
            }

            BigDecimal charge = units
                    .multiply(slab.getRatePerKl())
                    .setScale(MONEY_SCALE, RoundingMode.HALF_UP);

            waterCharge = waterCharge.add(charge);

            slabCharges.add(TariffSlabCharge.builder()
                    .from(from)
                    .to(to)
                    .units(units.setScale(
                            CONSUMPTION_SCALE,
        ...
        BigDecimal serviceCharge = findServiceCharge(
                consumption, slabs);

        BigDecimal totalWaterCharge = waterCharge
                .add(serviceCharge)
                .setScale(MONEY_SCALE, RoundingMode.HALF_UP);
        ...
        return TariffCalculationResult.builder()
                ...
                .consumption(consumption.setScale(
                        CONSUMPTION_SCALE,
                        RoundingMode.HALF_UP))
                .waterVolumetricCharge(waterCharge)
    ...
    private BigDecimal calculateUnitsInSlab(
            BigDecimal consumption,
            BigDecimal from,
            BigDecimal to) {
    ...
    private BigDecimal findServiceCharge(
            BigDecimal consumption,
            List<DJBMonthlyWaterTariffSlab> slabs) {

        /*
         * Tariff service charge is associated with the consumption slab.
         * Boundary handling is deliberately deterministic:
         * 20 KL -> the slab ending at 20;
         * 30 KL -> the slab ending at 30;
         * >30 KL -> the open-ended slab.
         */
        for (DJBMonthlyWaterTariffSlab slab : slabs) {
            ...
            if (to == null) {
                ...
                matches = consumption.compareTo(from) >= 0
                        && consumption.compareTo(to) <= 0;
            }

            if (matches) {
                return slab.getServiceCharge() == null
                        ? BigDecimal.ZERO.setScale(MONEY_SCALE)
                        : slab.getServiceCharge()
                            .setScale(MONEY_SCALE, RoundingMode.HALF_UP);
            }
        }
    ...
    private DJBMonthlyWaterTariff resolveTariff(
            String requestedCategory,
            List<DJBMonthlyWaterTariff> tariffs) {
        ...
        for (DJBMonthlyWaterTariff tariff : tariffs) {
            ...
            if (normalizeCategory(tariff.getCategory())
                    .equals(normalized)) {
                return tariff;
            }
        }
    ...
    private List<DJBMonthlyWaterTariffSlab> sortedActiveSlabs(
            DJBMonthlyWaterTariff tariff) {
    ...
    private String normalizeCategory(String value) {
        ...
        if ("RESIDENTIAL".equals(normalized)
                || "DOMESTIC_CONSUMER".equals(normalized)) {
            return "DOMESTIC";
        }
    ...
    private void validateConsumption(BigDecimal consumption) {
        if (consumption == null) {
            throw new IllegalArgumentException(
                    "Consumption is required for tariff calculation");
        }
